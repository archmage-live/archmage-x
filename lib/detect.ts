...
export const isExtensionContext = once(
  (): boolean => typeof globalThis.chrome?.extension === 'object'
)

/** Indicates whether the code is being run in a content script */
export const isContentScript = once(
  (): boolean => isExtensionContext() && isWebPage()
)
...
export const isBackgroundWorker = once((): boolean =>
  isCurrentPathname(getManifest(3)?.background?.service_worker)
)
...
export const isSafari = () =>
  !isChrome() && globalThis.navigator?.userAgent.includes('Safari')
...
export function getContextName(): ContextName | 'unknown' {
  for (const [name, test] of Object.entries(contextNames) as Array<
    [ContextName, BooleanFunction]
  >) {
    if (test()) {
      return name
    }
