...
import type { SafeAccountConfig } from '@safe-global/protocol-kit/dist/src/types'
...
import { Erc4337AccountType } from './types'
...
export enum WalletType {
  ...
  REOWN = 'wallet_connect', // WalletConnect protocol
  REOWN_GROUP = 'wallet_connect_group', // ditto, but in group
...
export function isGroupWallet(wallet: IWallet) {
  switch (wallet.type) {
    case WalletType.HD:
    // pass through
...
export function isKeystoreWallet(wallet: IWallet) {
  return hasMasterKeystore(wallet) || isKeystoreGroupWallet(wallet)
}
...
export function isKeystoreGroupWallet(wallet: IWallet) {
  switch (wallet.type) {
    case WalletType.PRIVATE_KEY_GROUP:
      return true
    default:
...
export function isReownWallet(wallet: IWallet) {
  switch (wallet.type) {
...
export function isSignableWallet(wallet: IWallet) {
  return (
...
function generateWalletUniqueHash() {
  return getAddress(dataSlice(keccak256(randomBytes(32)), 12))
}
