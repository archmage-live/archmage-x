...
import {
  ...
  getChecksumAddress,
  hash
} from 'packages/archmage/src/wallet/starknet'
import { encode } from 'packages/archmage/src/wallet/starknet'

import { braavosKey } from '@/archmage/crypto/braavosKey'
import {
  hdNodeFromKeystore,
  hdNodeFromPhrase
} from '@/archmage/crypto/secp256k1'
import { DerivePosition } from 'schema'
...
export class StarknetWallet implements KeystoreSigningWallet {
  ...
  private constructor(
    private wallet: HDNodeWallet | Wallet,
  ...
  static async from({
    ...
    return new StarknetWallet(
      wallet,
      encode.addHexPrefix(starknet.grindKey(wallet.privateKey))
    )
  }
  ...
  async derive(
    ...
    switch (STARKNET_ACCOUNT_TYPE) {
      case StarknetAccountType.ARGENT: {
        ...
        const walletForDefaultPath = hdNodeFromPhrase(
          this.wallet.mnemonic.phrase,
          '', // default path
          this.wallet.mnemonic.wordlist
        )
        // Second, we create a hd master node from the seed which is the private key of the wallet.
        const hdNode = HDNodeWallet.fromSeed(walletForDefaultPath.privateKey)
