...
import { Currency, useConvertQuery } from '@/archmage/datasource/uniswap'
import { FiatNumberType } from '@/archmage/format/types'
import { LocalizedFormatter } from '@/archmage/language/formatter'
import { PollingInterval } from '@/archmage/time/time'

import { FiatCurrency } from './constants'
import { getFiatCurrencyCode, useCurrentFiatCurrency } from './hooks'
...
export function useFiatConverter(
  ...
  const [appCurrency] = useCurrentFiatCurrency()
  const toCurrency = mapFiatCurrencyToServerCurrency[appCurrency || FiatCurrency.UnitedStatesDollar]
