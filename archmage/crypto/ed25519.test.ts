...
describe('ED25519 HDNode', () => {
  it('derive path', () => {
    ...
    const master = HDKey.fromMasterSeed(seed.slice(2))
    // console.log(master)

    for (const c of cases) {
      const derived = master.derive(c.path)
      expect(derived.parentFingerprintHex).toEqual(c.fingerprint)
      expect(bytesToHex(derived.chainCode)).toEqual(c.chainCode)
      expect(bytesToHex(derived.privateKey)).toEqual(c.privateKey)
      expect(bytesToHex(derived.publicKey)).toEqual(c.publicKey)
    }
  })
})
