import { mnemonicFromKeystore } from '@/archmage/crypto/mnemonic'
import {
  AminoSignResponse,
  Secp256k1HdWallet,
  Secp256k1Wallet,
...
export class CosmWallet implements KeystoreSigningWallet {
  ...
  static async from({
    ...
    if (type === WalletType.HD || type === WalletType.KEYLESS_HD) {
      ...
      wallet.mnemonic = mnemonicFromKeystore(keystore).phrase
      ...
      if (mnemonic) {
        ...
        const w = await DirectSecp256k1HdWallet.fromMnemonic(
          mnemonicFromKeystore(keystore).phrase,
          {
        ...
        wallet.privateKey = hexlify(account.privkey)
        wallet.publicKey = hexlify(account.pubkey)
      } else {
        assert(!path)
        const w = await DirectSecp256k1Wallet.fromKey(
          getBytes(keystore.privateKey),
          prefix
        )
        wallet.wallet = w
        wallet.prefix = prefix
  ...
  async derive(
    ...
    ws.privateKey = hexlify(account.privkey)
    ws.publicKey = hexlify(account.pubkey)
    return ws
  }
