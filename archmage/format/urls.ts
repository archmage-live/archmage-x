...
export function uriToHttpUrls(uri: string, options?: { allowLocalUri?: boolean }): string[] {
  ...
  switch (protocol) {
    case uri: {
      // If the result of protocol equals the uri, it means the uri has no protocol and is a local file (i.e. a relative or absolute path).
...
function truncateQueryParams(url: string): string {
  // In fact, the first element will always be returned below. url is
